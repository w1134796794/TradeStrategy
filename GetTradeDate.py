...
class LocalTradeCalendar:
    ...
    def get_recent_trade_dates(self,
                               base_date: str,
                               days: int,
                               direction: str = "before") -> List[str]:
        """
        获取指定日期附近的交易日列表
        ...
        if not self._validate_date_format(base_date):
            raise ValueError("base_date格式应为YYYYMMDD")
        ...
        end_idx = min(len(self.trade_dates) - 1, end_idx)
    ...
    def _find_date_index(self, date_str: str) -> int:
        """使用二分查找快速定位日期索引"""
        left, right = 0, len(self.trade_dates) - 1
        while left <= right:
            mid = (left + right) // 2
            if self.trade_dates[mid] == date_str:
                return mid
            elif self.trade_dates[mid] < date_str:
                left = mid + 1
            else:
                right = mid - 1
        return -1
